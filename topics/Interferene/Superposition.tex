...
\begin{tikzpicture}[scale=1.2]
    ...
    \draw[->, thick] (2, {0.75 - 2*tan(\angleVal)}) -- (P) node[pos=0.4, above, sloped] {Ray 1};
    
    \draw[->, thick] (2, {-0.75 - 2*tan(\angleVal)}) -- (Q) node[pos=0.4, above, sloped] {Ray 2};

    % Angle alpha label
    \draw[->] (0.7,0) arc (0:-\angleVal:-0.7);
    \node at (1.0, -0.25) {$\angleVal$};
